...
def build_prompt(metrics: list, category: str) -> str:
    ...
    return f"""Tu es un analyste business.
...
Sois précis, professionnel et oriente toujours vers l'action. Réponds en Anglais."""
...
def generate_insight_stream(metrics: list, category: str):
    if not metrics:
        raise ValueError(f"Aucune métrique trouvée pour la catégorie '{category}'")
